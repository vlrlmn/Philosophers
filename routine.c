...
/*   Updated: 2024/04/04 16:37:15 by vlomakin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long long	get_time_ms(struct timeval start)
{
	...
	elapsed = (now.tv_sec - start.tv_sec) * 1000LL + (now.tv_usec
	...
	while (1)
	{
		...
		while (i < table->ph_amount)
		{
			if (get_time_ms(table->timer)
				- table->philos[i].last_meal_time > (long long)table->time_to_die)
...
void	take_forks2(t_philo_args *table, pthread_mutex_t *fork1,
		pthread_mutex_t *fork2, int i)
{
...
void	take_forks(t_philo_args *table, pthread_mutex_t *fork1,
		pthread_mutex_t *fork2, int i)
{
	pthread_mutex_lock(fork1);
	change_status(table, "has taken a fork", i, 0);
...
void	*philo_routine(void *args)
{
	...
	{
		if (i % 2)
			take_forks(table, table->philos[i].left_fork,
					table->philos[i].right_fork, i);
		else
			take_forks2(table, table->philos[i].right_fork,
...
void	init_thread(t_philo_args *table)
{
	...
	while (i < table->ph_amount)
	{
		if (pthread_create(threads + i++, NULL, philo_routine, table))
	...
	if (pthread_create(dead_flag, NULL, check_if_dead, table))
		pthread_failed("Failed to create thread");
	while (i < table->ph_amount)
